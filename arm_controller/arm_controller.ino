...
// ---------------- POSES ----------------
// A station
int A_above[6] = {170,135,70,20,80,5};
int A_pick [6] = {170,160,50,10,70,5};
// B station
int B_above[6] = {90,135,70,20,80,5};
int B_pick [6] = {90,160,90,10,90,5};
...
void handoff(int pickPose[6], int abovePose[6]){
  ...
  // 3) LIFT ARM away from ball (before re-grabbing to avoid dragging)
  moveArmOnly(abovePose[0],abovePose[1],abovePose[2],abovePose[3],abovePose[4],12);
  delay(300);

  // 4) DESCEND again to re-grab cleanly
  moveArmOnly(pickPose[0],pickPose[1],pickPose[2],pickPose[3],pickPose[4],12);
  delay(400);

  // 5) CLOSE gripper to grab
  closeGrip();
  delay(300);

  // 6) LIFT AND TRAVEL to above pose
...
void loop(){
  ...
  if(!firstPickupDone){
    ...
    delay(300);
    moveArmOnly(A_pick[0],A_pick[1],A_pick[2],A_pick[3],A_pick[4],12);
